    ...
    for epoch in range(args.num_epoch):
        ...
        with tqdm(total=(len(train_dataset) - len(train_dataset) % args.batch_size)) as t:
            ...
            for data in train_dataloader:
                input_1,input_2,input_3,input_4,label_1,label_2,label_3,label_4 = data

                inputs = torch.stack((input_1,input_2,input_3,input_4),axis=1)
                labels = torch.stack((label_1,label_2,label_3,label_4),axis=1)
                inputs = inputs.to(device)
                labels = labels.to(device)

                preds = model(inputs)
                loss = nn.MSELoss(labels,preds)
        ...
        for data in eval_dataloader:
            ...
            preds = model(inputs)
            epoch_rmse.update(calc_rmse(preds[3],labels[3]),1)
